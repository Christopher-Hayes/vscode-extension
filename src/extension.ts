...
let projectDataProvider: any;
let workspaceFolderChangeListener: vscode.Disposable;
let outputChannel: vscode.OutputChannel;
let statusBar: vscode.StatusBarItem;
let currentProject: any;

async function selectProject(fileProvider: CloudStorageProvider): Promise<any> {
    ...
    const project = await vscode.window.showQuickPick(names, {
            ...
            for (const folder of folders) {
                if (folder.name === project) {
                    vscode.window.showErrorMessage(
                        'Project with identical name already added. Please rename.'
                    );
                    return;
                }
            }
...
function renameWorkspaceFolder(oldName: string, newName: string): void {
    ...
    const newUri = folderUri.with({
        path: folderUri.path.replace(oldName, newName)
    });
...
function displaySearchResults(results: any[]): void {
    ...
    const maxSearchResults = config.get<number>('maxSearchResults') ?? 1000;
    ...
    if (count === maxSearchResults) {
        outputChannel.appendLine(
            `Done. Displaying first ${maxSearchResults} results.`
        );
    } else {
    ...
    if (uri) {
        ...
        if (currentProject) {
            ...
            const branchName = data ?
                data.branch :
                await fileProvider.getProjectBranchName(currentProject);
...
async function switchBranch(project: any): Promise<void> {
    if (!project) {
        return;
    }
    ...
    try {
        ...
        const branch = await vscode.window.showQuickPick(names, {
...
export function activate(
    ...
    context.subscriptions.push(
        vscode.workspace.registerFileSystemProvider('playcanvas', fileProvider, {
    ...
    const handleNewWorkspaceFolder = (
    ...
    workspaceFolderChangeListener = vscode.workspace.onDidChangeWorkspaceFolders(
        (event) => {
        ...
        vscode.commands.registerCommand(
            ...
            async (item?: any) => {
                try {
                    ...
                    if (!token) {
                        vscode.window.showErrorMessage(
                            'Please generate your PlayCanvas access token.'
                        );
                        return;
                    }
                    ...
                    if (project) {
                        ...
                        const currentBranch = await fileProvider.getProjectBranchName(
                            project
                        );
                        const start = vscode.workspace.workspaceFolders ?
                    ...
                    if (!item) {
                        ...
                        if (editor) {
                            ...
                            vscode.window.showWarningMessage(
                                'Please use the command "PlayCanvas: Pull Latest" from the context menu.'
                            );
                            return;
                        }
                    }
            ...
            async (item?: any) => {
                if (!item) {
                    ...
                    if (editor) {
                        ...
                        // show warning if no project is selected
                        vscode.window.showWarningMessage(
                            'Please use the command "PlayCanvas: Switch Branch" from the context menu.'
                        );
                        return;
                    }
                }
                switchBranch(
                    item ? await fileProvider.getProject(item.path) : currentProject
                );
                updateStatusBarItem(statusBar);
            }
        )
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('playcanvas.search', async () => {
            // Get the currently selected text
            const editor = vscode.window.activeTextEditor;
            let project: any;
            let selectedText = '';
            let selectedPath: vscode.Uri | undefined;
            ...
            if (project) {
                outputChannel.appendLine(
                    `Searching for '${searchPattern}' in ${project.name}...`
                );
            } else {
            ...
            const results = await fileProvider.searchFiles(
                searchPattern,
                selectedPath
            );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(
            ...
            async (item: any) => {
                ...
                outputChannel.appendLine(
                    `Searching for '${searchPattern}' in ${item.path}...`
                );
                outputChannel.appendLine('');
    ...
    fileProvider.getToken().catch((err: any) => {
        vscode.window.showInformationMessage(
            `Failed to authorize. Error: ${err.message}`
        );
    });

    console.log(
        'playcanvas: Congratulations, your extension "playcanvas" is now active!'
    );
}
